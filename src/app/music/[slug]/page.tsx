...
import { tracks as skylineDriveLiveTracks } from '@/data/albums/skyline-drive-live'
...
const albumTracks: Record<string, typeof skylineDriveTracks> = {
  ...
  'skyline-drive-live': skylineDriveLiveTracks,
}
...
export default function AlbumPage() {
  ...
  // Set the current album when page loads
  useEffect(() => {
    if (slug && slug !== currentAlbum) {
      setCurrentAlbum(slug)
    }
  }, [slug, currentAlbum, setCurrentAlbum])
  ...
  // Check if we're viewing the same album that's currently playing
  const isCurrentAlbum = currentAlbum === slug
    ...
    <div className="min-h-screen bg-gradient-to-b from-owl-dark to-owl-midnight pt-24 pb-32">
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="text-center mb-8">
          ...
          {album.description && (
            <p className="text-owl-accent/60 mt-4 max-w-xl mx-auto italic">{album.description}</p>
          )}
        </div>
        ...
        <div className="bg-owl-slate/50 backdrop-blur-sm rounded-3xl p-8 mb-8 border border-owl-water/20">
          <div className="flex flex-col md:flex-row gap-8 items-center">
            {/* Album Art */}
            <div className="w-48 h-48 bg-owl-mist rounded-2xl overflow-hidden shadow-xl flex-shrink-0 relative">
              <img 
                src={album.cover}
                alt={album.title}
            ...
            <div className="flex-1 w-full">
              <div className="text-center md:text-left mb-6">
                <h2 className="text-2xl font-display text-white mb-1">
                  {isCurrentAlbum ? currentTrack.title : tracks[0]?.title || 'Select a track'}
                </h2>
                <p className="text-owl-accent/70">
                  {isCurrentAlbum ? currentTrack.artist : tracks[0]?.artist || 'Owls in Water'}
                </p>
              </div>
              ...
              <div className="mb-6">
                <input
                  ...
                  min="0"
                  max={duration || 100}
                  value={isCurrentAlbum ? progress : 0}
                  onChange={handleProgressChange}
                  className="audio-progress w-full h-2 bg-owl-mist rounded-full appearance-none cursor-pointer"
                  style={{
                    background: `linear-gradient(to right, #7eb8da ${isCurrentAlbum ? (progress / duration) * 100 || 0 : 0}%, #2d2d44 ${isCurrentAlbum ? (progress / duration) * 100 || 0 : 0}%)`
                  }}
                />
                <div className="flex justify-between text-sm text-owl-accent/60 mt-2">
                  <span>{formatTime(isCurrentAlbum ? progress : 0)}</span>
                  <span>{formatTime(isCurrentAlbum ? duration : 0)}</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-center gap-4">
                ...
                >
                  {isPlaying && isCurrentAlbum ? <Pause className="w-8 h-8" /> : <Play className="w-8 h-8 ml-1" />}
                </button>
            ...
            {tracks.map((track, index) => (
              <button
                ...
                className={`w-full px-4 py-3 flex items-center gap-4 hover:bg-owl-water/10 transition-colors text-left ${
                  isCurrentAlbum && index === currentTrackIndex ? 'bg-owl-water/20' : ''
                }`}
              >
                <div className="w-8 h-8 flex items-center justify-center text-owl-accent/50">
                  {isCurrentAlbum && index === currentTrackIndex && isPlaying ? (
                ...
                <div className="flex-1 min-w-0">
                  <p className={`truncate ${isCurrentAlbum && index === currentTrackIndex ? 'text-owl-glow' : 'text-white'}`}>
                    {track.title}
                  </p>
